...
## 2026-07-13 — Remote helper scripts defaulted to GPU 0 instead of allocated GPU 5  #mistake #decision
**Context:** issue #33 — running `scripts/setup_remote.sh` / `scripts/run_remote.sh` without
`TRINITY_GPU_INDEX` set.
**Expected:** remote workflows use GPU 5, matching `DEFAULT_TRINITY_GPU_INDEX` in validator config
and `secrets.env.example`.
...
`remote_env.sh`; added a regression test that `setup_remote.sh` logs GPU 5 when unset.
**Follow-up:** none.
