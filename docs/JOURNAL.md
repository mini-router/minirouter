...
**Follow-up:** `allow_toy_fallback` forwarding through the facade is tracked in #65 / PR #66.
...
## 2026-07-10 — Toy-set fallback made loud + opt-in (was silent)  #mistake #fix
**Context:** follow-up to #7 / closed PR #8 (issue #65 / PR #66): hardening `orchestration/dataset.load_tasks`
...
visibly; opt-in must work through the LiveCodeBench facade too.
**Actual:** `load_tasks` silently substituted `_toy_tasks()` whenever the HF load returned nothing; the
LiveCodeBench wrapper did not forward `allow_toy_fallback`; env guard only treated a narrow falsey set.
**Root cause:** the toy set is smoke-test-only, but nothing distinguished a smoke test from a real run,
and the public benchmark facade/env parsing did not fully propagate the opt-in guard.
**Fix / decision:** `load_tasks` now raises by default; toy fallback is opt-in via
`allow_toy_fallback=True` or `TRINITY_ALLOW_TOY_FALLBACK` (with case-insensitive falsey values like
`false`/`no`/`off`); `benchmarks/livecodebench.py` forwards the flag; smoke test S8 opts in explicitly.
Covered by `tests/test_dataset_fallback.py` and `tests/test_livecodebench.py`.
