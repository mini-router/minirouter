"""Tests for the toy-set fallback guard in ``load_tasks`` (issue #7).
...
def test_opt_in_param_returns_toy_and_warns(monkeypatch, _clear_env, capsys):
    ...
    assert tasks  # toy set is non-empty
...
def test_successful_load_is_unaffected(monkeypatch, _clear_env, capsys):
    # A non-empty real load must pass straight through: no raise, no toy, no warning.
    real = D._toy_tasks("math500")[:1]  # stand-in for "real" tasks (non-empty list)
    monkeypatch.setitem(D._HF_LOADERS, "math500", lambda split: list(real))
    tasks = D.load_tasks("math500", "test", max_items=None)
    assert len(tasks) == 1
    assert capsys.readouterr().err == ""  # no fallback warning emitted
