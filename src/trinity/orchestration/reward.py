...
def normalize_math_answer(ans: str | None) -> str:
    ...
    if s.startswith("="):
        s = s[1:].strip()
    # Strip a single outer pair of \{ \} or { }. Use a non-greedy capture so
    # the optional trailing \\? can claim the LaTeX escaping backslash on
    # \{...\} (greedy (.*) swallowed it and left a stray trailing \\).
