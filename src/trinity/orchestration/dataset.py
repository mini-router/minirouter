...
# Env values treated as "do not allow toy fallback" (case-insensitive).
_TOY_FALLBACK_FALSEY = frozenset({"0", "false", "no", "off", "n"})
...
def _toy_fallback_allowed(explicit: bool) -> bool:
    ...
    if explicit:
        return True
    raw = os.environ.get(_TOY_FALLBACK_ENV, "").strip()
    if not raw:
        return False
    return raw.lower() not in _TOY_FALLBACK_FALSEY
